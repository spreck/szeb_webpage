...
class GeoServerConnection {
  constructor(config = window.geoServerConfig) {
    this.config = config;
    this.cache = new Map();
    ...
    if (this.config.getConfig().enableHealthCheck) {
  ...
  async executeWithRetry(requestFn, options = {}) {
    const maxRetries = options.retries || this.config.getRetryAttempts();
      ...
      try {
        ...
        if (cacheKey) {
          this.cache.set(cacheKey, {
            data: result,
            expiration: Date.now() + this.config.getCacheExpiration()
          });
        }
    ...
    return this.executeWithRetry(async () => {
      ...
      try {
        ...
        if (error.name === 'AbortError') {
          throw new Error(`WFS request timed out after ${this.config.getTimeout()}ms`);
        }
        throw error;
      }
    }, {
  ...
  async getFeatureInfo(layerName, latlng, map, options = {}) {
    ...
    return this.executeWithRetry(async () => {
      ...
      try {
        ...
        if (error.name === 'AbortError') {
          throw new Error(`GetFeatureInfo request timed out after ${this.config.getTimeout()}ms`);
        }
        throw error;
      }
    }, options);
